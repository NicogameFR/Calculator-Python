...
if 'ipykernel' in sys.modules:
    ...
    def _init_ui():
        ...
        _vlayout = widgets.Layout(
            flex_flow="column nowrap", align_items="center")
        _root = widgets.HBox([], layout=_hlayout)

        _side = 'left'
        _frames = []    # la pile des conteneurs
        _sides = []     # l'orientation associ̩e
    ...
    def end_horizontal():
        ...
        global _root, _side
        if (_side != 'left'):
            print('end_horizontal() dans un conteneur vertical: ignor̩')
        if len(_frames) == 0:
            print('end_horizontal() sans conteneur ouvert: ignor̩')
            return
        _root = _frames.pop()
        _side = _sides.pop()
    ...
    def end_vertical():
        ...
        global _root, _side
        if (_side != 'top'):
            print('end_vertical() dans un conteneur horizontal: ignor̩')
        if len(_frames) == 0:
            print('end_vertical() sans conteneur ouvert: ignor̩')
            return
        _root = _frames.pop()
        _side = _sides.pop()
    ...
    def slider(label, from_, to, command=None):
        ...
        if type(from_) is int and type(to) is int:
            s = widgets.IntSlider(
                min=from_, max=to, readout=False, layout=slayout)
        else:
        ...
        def set_value(val):
            set_text(v, val)
            val = (val - from_)/(to - from_) * \
                (int(slayout.width[:-2]) - 9*len(str(to)))
    ...
    def set_width(w, width):
        ...
        if isinstance(w, widgets.widget_int.IntSlider) or isinstance(w, widgets.widget_float.FloatSlider):
            ...
            w.value = w.value
        else:  # largeur en caract̬res (approximativement)
    ...
    # il vaut mieux utiliser start_ui() par souci de coh̩rence
    def main_loop():
    ...
    def button(title, command, param=None):
        ...
        if param is None:
            ...
            b = tk.Button(_root, text=title,
                          command=_button_command(command, param))
        b.pack(side=_side)
        return b
    ...
    def update_options(om, list):
        ...
        for item in list:
            om['menu'].add_command(
                label=item, command=tk._setit(om._optionvar, item))
